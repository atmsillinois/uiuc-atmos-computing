...
+---------+--------------------------------------------+------------------+
| gpu     | Access to GPU (K40m), MaxNodes=1           | 7 days           |
+---------+--------------------------------------------+------------------+
...
+------------+----------------+----------------------+
| Name       | Cores per node | Real memory (MB)     |
+------------+----------------+----------------------+
| a          | 4              | 12000                |
+------------+----------------+----------------------+
| b          | 8              | 16000, 32000         |
...
Keeling has 5 K40m GPU nodes and 8 L40S GPUs. To access these resources, you must specify the
``gpu`` or ``L40S`` partition when requesting an interactive session.

.. note::

   The ``gpu`` partition (K40m) is legacy hardware down to a single node and may be removed
   in the future. Use the ``L40S`` partition for new workflows.

To interactively compute on a K40m GPU node:

.. code-block:: console

    qlogin -p gpu -n 20 --gres=gpu:K40m:1 --mem=256000

To interactively compute on a newer L40S GPU node:
